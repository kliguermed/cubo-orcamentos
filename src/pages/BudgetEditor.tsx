...
const BudgetEditor = () => {
  ...
  const fetchItems = async (envId: string) => {
    try {
      const { data, error } = await supabase.from("items").select("*").eq("environment_id", envId);
      if (error) throw error;
      setItems(data || []);
    } catch (error: any) {
      toast({
  ...
  const addEnvironmentFromTemplate = async (templateName: string, templateId?: string) => {
    try {
      ...
      if (id) {
        ...
        const { data: updatedEnvs } = await supabase.from("environments").select("*").eq("budget_id", id);
        if (updatedEnvs) {
  ...
  const saveEnvironment = async (environment: Environment) => {
    try {
      ...
      const { error } = await supabase.from("environments").update(updateData).eq("id", environment.id);
      if (error) throw error;
  ...
  const saveItem = async (itemData: Omit<Item, "id">) => {
    try {
      ...
      if (editingItem) {
        ...
        // Create new item
        const { error } = await supabase.from("items").insert([dataToSave]).select();
        if (error) throw error;
        toast({
          title: "Item adicionado",
          ...
          <div className={isMobile ? "space-y-4" : "lg:col-span-1 space-y-6"}>
            ...
            <Card>
              <CardHeader
                ...
                onClick={() => setCalculationRulesExpanded(!calculationRulesExpanded)}
              >
                <div className="flex justify-between items-center">
              ...
              <CardContent>
                <div className="space-y-2 mb-4">
                  {environments.map((env) => (
                    <div
                      key={env.id}
                      className={`p-3 border rounded-md cursor-pointer transition-colors ${selectedEnvId === env.id ? "bg-accent" : "hover:bg-accent/50"}`}
                ...
                <CardContent>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              ...
                              <div>
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    updateItem(item.id, "purchase_price", parseFloat(e.target.value) || 0)
                                  }
                                  onFocus={(e) => e.target.select()}
                  ...
                  {items.length > 0 && (
                    <div className="mt-6 p-3 sm:p-4 bg-muted/50 rounded-lg">
                      <h3
                        className={isMobile ? "text-base font-semibold mb-3 text-center" : "text-lg font-semibold mb-4"}
                      >
                        Resumo do Ambiente
                      </h3>
                      <div className={isMobile ? "grid grid-cols-2 gap-3" : "grid grid-cols-2 md:grid-cols-3 gap-4"}>
                        <div className="text-center">
                          ...
                          <div className={isMobile ? "text-sm font-semibold" : "text-lg font-semibold"}>
                            {formatCurrency(environmentTotals.purchaseTotal)}
                          </div>
                        </div>
                        <div className="text-center">
                          <div className={isMobile ? "text-xs text-muted-foreground" : "text-sm text-muted-foreground"}>
                          ...
                          <div
                            className={
                              isMobile ? "text-sm font-semibold text-blue-600" : "text-lg font-semibold text-blue-600"
                            }
                          >
                            {formatCurrency(environmentTotals.laborTotal)}
                          </div>
                        </div>
                        <div className="text-center">
                          <div className={isMobile ? "text-xs text-muted-foreground" : "text-sm text-muted-foreground"}>
                          ...
                          <div
                            className={
                              isMobile ? "text-sm font-semibold text-green-600" : "text-lg font-semibold text-green-600"
                            }
                          >
                            {formatCurrency(environmentTotals.rtTotal)}
                          </div>
                        </div>
                        <div className="text-center">
                          <div className={isMobile ? "text-xs text-muted-foreground" : "text-sm text-muted-foreground"}>
                          ...
                          >
                            {formatCurrency(environmentTotals.costTotal)}
                          </div>
                        </div>
                        <div className="text-center">
                          <div className={isMobile ? "text-xs text-muted-foreground" : "text-sm text-muted-foreground"}>
                          ...
                          >
                            {formatCurrency(environmentTotals.profitTotal)}
                          </div>
                        </div>
                        <div className="text-center">
                          <div className={isMobile ? "text-xs text-muted-foreground" : "text-sm text-muted-foreground"}>
                          ...
                          <div
                            className={isMobile ? "text-lg font-bold text-primary" : "text-xl font-bold text-primary"}
                          >
                            {formatCurrency(environmentTotals.finalTotal)}
                          </div>
                        </div>
                      </div>
                    </div>
